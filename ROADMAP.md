...
- client-side filter ตามชื่อ/หมวดหมู่

### 2.4 ~~Category filter~~ ✅
- คลิกหมวดหมู่ → filter แสดงเฉพาะช่างในหมวดนั้น (toggle chip)

### 2.5 ~~Empty state & Loading skeleton~~ ✅
- skeleton loader ตอนดึงพิกัด + empty state เมื่อไม่พบช่าง + ปุ่มล้างตัวกรอง
